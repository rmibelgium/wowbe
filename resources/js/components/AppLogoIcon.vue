<script setup lang="ts">
import { getActiveLanguage } from 'laravel-vue-i18n';
import type { HTMLAttributes } from 'vue';

defineOptions({
    inheritAttrs: false,
});

interface Props {
    className?: HTMLAttributes['class'];
}

defineProps<Props>();

const lang = getActiveLanguage();
</script>

<template>
    <svg width="48" height="60" viewBox="0 0 48 60" xmlns="http://www.w3.org/2000/svg">
        <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
            <g transform="translate(-762 -260)">
                <g transform="translate(630 260)">
                    <g transform="translate(132)">
                        <path fill="#FFF" d="M0 0h47.273v60H0z" />
                        <g transform="translate(4.066 4.5)">
                            <path fill="#0056EF" d="M.114 39.297h39.177V.787H.114z" />
                            <path
                                fill="#FFF"
                                d="M10.508.393l.857 1.012 1.313 1.574 1.428 1.968.742 1.125.743 1.18.685 1.18.628 1.294.571 1.237.4 1.293.172.618.17.675.058.618.057.619.057 1.405v2.643l-.114 1.18-.114 1.068-.229 1.069-.228 1.011-.286.956-.342.9-.343.9-.4.899-.456.843-1.085 1.8-1.143 1.854-.628 1.012-.457 1.012-.4 1.012-.285 1.012-.229.956-.171.956-.114.9v2.36l.114 1.18.171.732.058.281 2.112-.169-.057-.225v-1.462l.115-1.067.057-.675.171-.618.114-.675.229-.731.285-.73.286-.732.4-.674.457-.731 1.142-1.574 1.085-1.35 1.085-1.18 1.028-1.012 1.827-1.8L23.7 25.75l.686-.787.685-.844.571-.9.571-1.011.228-.563.229-.562.171-.618.229-.675.342-1.517.229-1.743.114-.675.114-.675.171-.618.172-.619.457-1.236.571-1.237.628-1.125.743-1.124.8-1.012.856-.956.913-.9.971-.786.971-.731 1.028-.618 1.028-.506.971-.394.514-.169.514-.112.514-.113.457-.056.514-.112.4-.113.228-.168.114-.169.058-.169-.058-.225-.057-.225-.171-.224-.4-.394-.4-.393-.342-.225-.172-.113L10.165 0z"
                            />
                            <template v-if="lang == 'fr'">
                                <path fill="#0056EF" d="M0 52.397h2.398v-11.75H0z" />
                                <path
                                    d="M10.28 40.647H6.225v11.75h2.341v-5.229h2.285l.4.057.342.056.343.168.286.17.285.28.285.394 2.57 4.104h2.685l-3.141-4.723-.172-.225-.171-.224-.229-.169-.17-.169-.172-.112-.229-.112-.228-.057-.229-.112.686-.169.628-.169.628-.28.457-.282.457-.394.286-.393.114-.225.114-.28v-.226l.057-.28-.057-.507-.114-.45-.172-.393-.228-.337-.286-.281-.342-.282-.4-.224-.4-.17-.457-.168-.514-.112-.57-.057-.515-.112-1.199-.056h-1.2zm-1.714 1.462h2.285l.8.056.685.056.57.169.458.168.342.281.286.281.171.338.057.393-.057.17-.057.224-.057.169-.114.168-.286.337-.4.225-.513.225-.629.169-.685.112H8.566v-3.54z"
                                    fill="#0056EF"
                                />
                                <path
                                    fill="#0056EF"
                                    d="M35.35 40.647l-5.082 9.782-5.197-9.782h-3.94v11.75h2.398V42.221h.057l5.426 10.176h2.341l5.482-10.176h.058v10.176h2.341v-11.75z"
                                />
                            </template>
                            <template v-else-if="lang == 'nl'">
                                <path
                                    fill="#0056EF"
                                    d="M9.238 40.595l-6.885 5.173v-5.173H0v11.751h2.353v-5.735l7.172 5.735h3.385l-8.205-6.185 7.63-5.566z"
                                />
                                <path
                                    fill="#0056EF"
                                    d="M29.205 40.595l-4.935 9.783-5.106-9.783h-3.787v11.751h2.295V42.17h.057l5.336 10.176h2.295l5.336-10.176v10.176h2.353V40.595z"
                                />
                                <path fill="#0056EF" d="M37.18 52.346h2.352V40.595H37.18z" />
                            </template>
                            <template v-else>
                                <path
                                    fill="#0056EF"
                                    d="M4.049 40.487H0v11.715h2.348v-5.18h2.673l.324.08.323.16.325.238.323.24.243.398 2.51 4.064h2.754L8.665 47.5l-.162-.239-.243-.16-.162-.239-.162-.079-.162-.16-.243-.079-.243-.08-.243-.08v-.08l.73-.08.647-.238.567-.24.486-.318.405-.399.324-.398.161-.478.082-.478v-.558l-.163-.398-.161-.4-.243-.318-.243-.319-.324-.24-.405-.238-.405-.24-.972-.238-1.134-.16-1.134-.079H4.05zm-1.7 1.434h2.267l.81.08.647.08.568.16.485.159.324.239.324.318.162.32v.398l-.08.398-.163.399-.243.319-.405.238-.567.16-.567.16-.728.159H2.348V41.92z"
                                />
                                <path
                                    fill="#0056EF"
                                    d="M28.829 40.487l-5.101 9.723-5.183-9.723h-3.888v11.715h2.349V42.081l5.506 10.121h2.349l5.426-10.121h.08v10.121h2.35V40.487z"
                                />
                                <path fill="#0056EF" d="M36.927 52.202h2.349V40.487h-2.349z" />
                            </template>
                        </g>
                    </g>
                </g>
            </g>
        </g>
    </svg>
</template>
